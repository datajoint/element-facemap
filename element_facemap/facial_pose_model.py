...
class FacemapProcessing(dj.Computed):
    ...
    def key_source(self):
        ...
        return FacemapPoseEstimationTask & fbe.VideoRecording.File

    def make(self, key):
        """Runs Facemap"""

        task_mode = (FacemapPoseEstimationTask & key).fetch1("task_mode")

        output_dir = (FacemapPoseEstimationTask & key).fetch1("facemap_output_dir")
        if not output_dir:
            ...
            FacemapPoseEstimationTask.update1(
                {**key, "facemap_output_dir": output_dir.as_posix()}
            )

        if task_mode == "trigger":
            from facemap import utils
            from facemap.pose import refine_pose, model_loader, pose as facemap_pose

            params = (FacemapPoseEstimationTask & key).fetch1("facemap_params")
            ...
            # MAKE THIS A FULL PATH USING FIND_FULL_PATH
            ...
            model_name = (FacemapPoseEstimationTask & key).fetch("model_name")
            # Fetches file attachment
            facemap_model_path = (
                FacemapModel.File & f'model_name="{model_name}"'
            ).fetch("file")
            ...
            shutil.copy(
                find_full_path(get_facemap_root_data_dir(), facemap_model_path),
                models_root_dir,
            )
            ...
            pose = facemap_pose.Pose(
                ...
                bbox=(FacemapPoseEstimationTask & key).fetch1(
                    "bbox"
                ),  # should be manually inserted into params via jupyter notebook cell
                bbox_set=(FacemapPoseEstimationTask & key).fetch1("bbox_set"),
                gui=None,
                GUIobject=None,
            ...
            keypoints_filepath = find_full_path(get_facemap_root_data_dir(), output_dir)
...
class FacemapPoseEstimation(dj.Computed):
    ...
    def make(self, key):
        ...
        if task_mode == "trigger":
            ...
            facemap_model_path.copy(facemap_model_path, models_root_dir)
            model_output_path = Path(models_root_dir) / facemap_model_name
            # copy using pathlib
            model_output_path.write_text(facemap_model_path.read_text())
            ...
            facemap_result = utils.load_keypoints(
            ...
            keypoints_data = utils.load_keypoints(
                refine_pose.BODYPARTS, keypoints_filepath
            )
            ...
            for b_idx, bodypart in enumerate(refine_pose.BODYPARTS):
                entry = {
                    "body_part": bodypart,
            ...
            #     {
            #         **key,
            #         "body_part": metadata["bodyparts"][y_],
            #         "frame_index": facemap_result[x_][y_][:],
            #         "x_pos": facemap_result[0][y_][likelihood_],
            #         "y_pos": facemap_result[1][y_][likelihood_],
            #         "likelihood": facemap_result[2][y_][likelihood_],
            #     }
            #     for x_ in range(facemap_result.shape[0])
            #     for  in range(facemap_result.shape[1])
            #     for likelihood_ in range(facemap_result.shape[2])
            # ]
